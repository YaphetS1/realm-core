...
{
    if (ndx >= m_size) {
        throw ExceptionForStatus(ErrorCodes::InvalidatedObject, "Object was deleted");
    }
    ClusterNode::State state;
...
size_t ClusterTree::Iterator::get_position()
{
    ...
    if (ndx == realm::npos) {
        throw ExceptionForStatus(ErrorCodes::LogicError, "Outdated iterator");
    }
    return ndx;
}
...
void ClusterTree::Iterator::go(size_t abs_pos)
{
    ...
    if (abs_pos >= sz) {
        throw ExceptionForStatus(ErrorCodes::OutOfBounds, "Index out of range");
    }
